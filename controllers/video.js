...
export const deleteVideo = async (req, res, next) => {
    try {
        ...
        if (req.user.id === video.userId) {
            ...
            res.status(200).send(
                `Video with videoId: ${deletedVideo.videoId} was successfully deleted!`
            );
        } else {
...
export const getTrendingVideos = async (req, res, next) => {
    try {
        ...
        const trendingVideos = await axios.get(
            `https://youtube.googleapis.com/youtube/v3/videos?part=snippet%2CcontentDetails%2Cstatistics&chart=mostPopular&maxResults=15&regionCode=IN&key=${process.env.YOUTUBE_API_KEY}`
        );
        res.status(200).send(trendingVideos?.data?.items);
    } catch (err) {
        next(err);
    }
};
